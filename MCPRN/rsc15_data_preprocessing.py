...
def load_data_rsc15( file):     
    ...
    print("Data selection for last 30 days")
    tmax = data.Time.max()
    last30Days = tmax - (86400 * 30)
...
def filter_data_rsc15( data, min_item_support=MINIMUM_ITEM_SUPPORT, min_session_length=MINIMUM_SESSION_LENGTH ): 
    ...
    while (counter):
        ...
        if min(session_group) >= min_session_length and min(item_group) >= min_item_support:
            counter = False
        """
        if min(session_group) >= min_session_length:
            counter = False
...
def split_list(input_list, split_ratio=0.7): # 70% data for traning and 30% for testings....
...
def split_data_rsc15( data, tes_days = DAYS):
    ...
    session_more_than1 = [key for key, value in session_group.items() if value >= MINIMUM_SESSION_LENGTH]
    ...
    item_no = item_no +1
    return [features_train, targets_train], [features_test, targets_test], item_no, train, test
