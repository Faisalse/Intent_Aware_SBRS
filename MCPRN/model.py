...
def train_validation(epoch_, model, train_data, test_data, accuracy_dictionary, patience):

    for epoch in tqdm(range(epoch_)):
        print("Epoch:  "+str(epoch))
        for key in  accuracy_dictionary:
            accuracy_dictionary[key].reset()
        ...
        for i in slices:
            ...
            indexes = scores.topk(20)[1]
            scores = scores.topk(20)[0]
            indexes = trans_to_cpu(indexes).detach().numpy()
            scores = trans_to_cpu(scores).detach().numpy()

            for i in range(len(targets)):
                target = targets[i] - 1
                recommendation_series = pd.Series(scores[i], index = indexes[i])
                recommendation_series.sort_values(ascending = False,  inplace = True)

                for key in  accuracy_dictionary:
                    accuracy_dictionary[key].add(recommendation_series, target)

        earlystopping(accuracy_dictionary["MRR_20"].score(), epoch)
        print("Best MRR@20 Score:   "+str(earlystopping.best_score))
        print("Current MRR@20 Score:   "+str(accuracy_dictionary["MRR_20"].score()))
...
def train_test(epoch_, model, train_data, test_data, accuracy_dictionary):
